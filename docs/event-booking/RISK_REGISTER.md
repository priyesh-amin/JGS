...
|---|---|---:|---:|---|---|
| R1 | Incorrect member-to-email migration lets the wrong person access an account | Medium | Critical | Exact authorised email allowlist; Google `sub` binding only when authoritative; mailbox reset proof for third-party domains; import provenance/audit | Roster reconfirmed: 56 unique emails; activation/import pending |
...
| R9 | D1 migration or deployment disrupts the existing public site | Low | High | Additive migration; preview first; backup/export; recorded commit; UI feature cutover separate | `0004` applied after verified backup; existing two-account invariant preserved |
...
| R15 | Google account email is not authoritative for a third-party mailbox | Medium | High | Auto-link only Gmail/Googlemail or verified Workspace (`hd`); otherwise require password/reset proof before linking | Mitigated locally; production identity UAT pending |
| R16 | Reset link is guessed, replayed, leaked or used for enumeration | Low | Critical | 256-bit token; hash-only D1 storage; 60-minute single use; fragment URL; generic response; throttling; session revocation | Mitigated locally; production mail UAT pending |
