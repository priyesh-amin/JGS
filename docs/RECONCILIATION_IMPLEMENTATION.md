...
2. Validate setup against an isolated database. The authenticated **Admin → Reconciliation → Enable reconciliation** action applies the fixed migrations `0006` and `0007` atomically through the existing D1 binding, records migration names and audits the action. It accepts no SQL from the request. Repeated setup is a no-op. Normal reads do not auto-migrate finance tables, and setup never activates member accounts or posts financial entries. Deployment workflows perform no D1 administration.
...
6. After release authorisation, deploy and enable the reviewed workspace through the protected setup action. After financial acceptance, activate only reconciled member accounts. Record who imports/reviews bank data and how often. Existing accounts continue using legacy balances until individually approved.
...
Final local verification: `npm run check` passed all 160 tests, release policy, lint, frontend build and Pages Functions compilation. New tests cover automatic booking-to-bank-payment settlement, unchanged historical invoices during migration, fee/due-date/cancellation-term snapshots, cancellation review and release, rebooking, free/unknown fees, opening cutoffs, stale event prices, atomic rollback and member privacy. Two subagents contributed the interface/configuration updates and independent lifecycle tests; their changes were integrated and checked together.

Seven setup tests additionally verify source-SQL parity, full rollback including DDL, repeat setup, upgrade from the finance foundation, unchanged legacy balances and bookings, no automatic account activation, and authenticated same-origin administrator access.
